...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        # Choose between velocity field (Neural ODE) or displacement field (4DGS)
        # Velocity field: Learns dx/dt = v(x,t) → continuous dynamics
        # Displacement field: Learns Δx = f(x,t) → independent per-timestep
        self.use_velocity_field = getattr(args, 'use_velocity_field', False)
        if self.use_velocity_field:
            print("🚀 Using VELOCITY FIELD (Neural ODE) for continuous dynamics")
            self._deformation = velocity_network(args)
        else:
            print("📍 Using DISPLACEMENT FIELD (4DGS) for per-timestep deformation")
        ...
        self.clone_freeze_steps = 0
        self.clone_birth_iter = torch.empty(0)
    ...
    def capture(self):
        return (
            ...
            self.clone_birth_iter,
        )
    
    def restore(self, model_args, training_args):
        clone_birth_iter = None
        ...
        # CRITICAL: If we're switching from displacement to velocity field (or vice versa),
        # DON'T load the deformation state (different architectures!)
        try:
            self._deformation.load_state_dict(deform_state)
            print("✓ Loaded deformation state")
        except (RuntimeError, AttributeError) as e:
            print(f"⚠️  Deformation state mismatch (expected when switching field types): {e}")
            print("   Continuing with randomly initialized deformation network")
        
        self.training_setup(training_args)
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        if clone_birth_iter is not None:
            self.clone_birth_iter = clone_birth_iter
        else:
            self.clone_birth_iter = torch.full((self._xyz.shape[0], 1), -1e6, device="cuda")
        
        # Try to load optimizer state, but skip if it doesn't match (e.g., switching field types)
        try:
            self.optimizer.load_state_dict(opt_dict)
            print("✓ Loaded optimizer state")
        except (ValueError, RuntimeError) as e:
            print(f"⚠️  Optimizer state mismatch (expected when switching networks): {e}")
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self.clone_birth_iter = torch.full((self.get_xyz.shape[0], 1), -1e6, device="cuda")
    ...
    def training_setup(self, training_args):
        ...
        if self.clone_birth_iter.numel() == 0 or self.clone_birth_iter.shape[0] != self.get_xyz.shape[0]:
            self.clone_birth_iter = torch.full((self.get_xyz.shape[0], 1), -1e6, device="cuda")
        else:
            self.clone_birth_iter = self.clone_birth_iter.to("cuda")
    ...
    def prune_points(self, mask):
        ...
        if self.clone_birth_iter.numel() == valid_points_mask.shape[0]:
            self.clone_birth_iter = self.clone_birth_iter[valid_points_mask]
        elif self.clone_birth_iter.numel() != 0:
            # Fallback: resize to match
            self.clone_birth_iter = self.clone_birth_iter[:valid_points_mask.shape[0]]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, birth_iterations):
        d = {"xyz": new_xyz,
        ...
        # CRITICAL FIX: Extend all accumulators for new Gaussians (otherwise shape mismatch in prune_points)
        self._deformation_accum = torch.cat([self._deformation_accum, torch.zeros((new_xyz.shape[0], 3), device="cuda")], dim=0)
        ...
        if self.clone_birth_iter.numel() == 0:
            self.clone_birth_iter = torch.full((optimizable_tensors["xyz"].shape[0] - new_xyz.shape[0], 1), -1e6, device="cuda")
        self.clone_birth_iter = torch.cat([self.clone_birth_iter, birth_iterations], dim=0)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, iteration=None):
        ...
        current_iter = iteration if iteration is not None else getattr(self._deformation, 'current_iteration', 0)
        birth_iterations = torch.full((new_xyz.shape[0], 1), float(current_iter), device="cuda")
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_deformation_table, birth_iterations)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        current_iter = iteration if iteration is not None else getattr(self._deformation, 'current_iteration', 0)
        birth_iterations = torch.full((new_xyz.shape[0], 1), float(current_iter), device="cuda")
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, birth_iterations)
    ...
    def add_point_by_mask(self, selected_pts_mask, perturb=0, iteration=None):
    ...
    def get_velocity_freeze_mask(self):
        return None
